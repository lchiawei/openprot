...
impl<S: embedded_hal::spi::SpiDevice> SpiFlash<S> {
    ...
    pub fn init(&mut self) -> Result<(), ErrorCode> {
        ...
        // Configure opcodes based on capacity (use 4-byte opcodes if > 16MB)
        if size <= 16 * 1024 * 1024 {
            ...
            self.read_cmd = SfCmd {
                opcode: 0x13, // Read Data 4B
                addr_bytes: 4,
                dummy_bytes: 0,
            };
            self.program_cmd = SfCmd {
                opcode: 0x12, // Page Program 4B
                addr_bytes: 4,
                dummy_bytes: 0,
            };
            self.erase_4k_opcode = 0x21; // Sector Erase 4KB 4B
            self.erase_64k_opcode = 0xDC; // Block Erase 64KB 4B
